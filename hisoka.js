	...
	break
	    case 'setname': case 'setsubject': {
            ...
            case 'pinterest': {
                ...
                hisoka.sendMessage(m.chat, { image: { url: result }, caption: '            *Succes*            : '+result }, { quoted: m })
            }
            break
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'owner': case 'creator': {
                let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    ...
                    + 'TEL;type=CELL;type=VOICE;waid=6281523940672:+62 882-9202-4190\n' // WhatsApp ID + phone number
                    + 'END:VCARD'
                hisoka.sendMessage(m.chat, { contacts: { displayName: 'FaxTzy XZ.', contacts: [{ vcard }] } }, { quoted: m })
            }
            break
            case 'eval': {
            ...
            case 'tes': case 'menu': case 'help': case '?': {
                anu = `${monospace}Hi ${pushname}${monospace}
                ...
                const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
                    templateMessage: {
                        hydratedTemplate: {
                            ...
                            hydratedButtons: [{
                                urlButton: {
                                    displayText: 'Github Dev',
                                    url: 'https://github.com/FaxDev'
                                }
                            }, {
                                callButton: {
                                    displayText: 'Number Phone Owner',
                                    phoneNumber: 'wa.me/6281523940672'
                                }
                            }, {
                                quickReplyButton: {
